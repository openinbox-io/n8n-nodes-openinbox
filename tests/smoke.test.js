...
test('package.json declares an `n8n` block with credentials and nodes', () => {
	...
	assert.ok(Array.isArray(pkg.n8n.credentials) && pkg.n8n.credentials.length > 0);
...
test('every entry in `n8n.credentials` and `n8n.nodes` exists on disk', () => {
	for (const rel of [...pkg.n8n.credentials, ...pkg.n8n.nodes]) {
		...
		assert.ok(fs.existsSync(abs), `Missing build output: ${rel}. Did you run \`npm run build\`?`);
	}
});
...
test('OpenInboxApi credential exports a class with required fields', () => {
	...
	assert.equal(typeof inst.displayName, 'string', 'credential.displayName must be a string');
	assert.ok(Array.isArray(inst.properties) && inst.properties.length > 0, 'credential.properties must be a non-empty array');
});
...
for (const rel of pkg.n8n.nodes) {
	...
	test(`${file} loads and exposes a valid node description`, () => {
		...
		assert.ok(Array.isArray(desc.group) && desc.group.length > 0, 'description.group must be a non-empty array');
